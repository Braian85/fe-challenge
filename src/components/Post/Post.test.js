...
const resdata = 
 ...
 it('should render first post and check if title appears', () => {
    const {getByText} = render(<Post post= {resdata} activatedComments = {false} />)
...
it('should check if a comment exists when comment is activated.', () => {
    const {getByText} = render(<Post post= {resdata} activatedComments = {true} />)
    expect(getByText('Hello')).toBeInTheDocument()
}); 

it('should check comment does not exist if comments are not activated.', () => {
    const {queryByText} = render(<Post post= {resdata} activatedComments = {false} />)
